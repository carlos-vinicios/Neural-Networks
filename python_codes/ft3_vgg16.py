...
pasta_raiz = "Base/" #define a pasta raiz contendo os splits para treino da rede
model_save_name = "FT_VGG16" #define o nome principal para salvar o modelo
model_save_dest = "Pesos" #define o destino para salvar o modelo
...
for split in range(1, 6): #laço de repetição para atingir todos os diretórios com imagens para treino, validação e teste
  metricas = {}
  json_name = "run_split_"+str(split)+".json"
  train_data_dir = trains_dir[split-1]
  test_data_dir = tests_dir[split-1]
  validation_data_dir = validations_dir[split-1]
  ...
  print()
  acuracia = metrics.accuracy_score(labels, preds_rounded)
  metricas["Acuracia"] = acuracia
  print(acuracia) #calcula o acurácia
  ...
  metricas["TP"] = confusion[0][0]
  metricas["FP"] = confusion[0][1]
  metricas["FN"] = confusion[1][0]
  metricas["TN"] = confusion[1][1]

  metricas["Report"] = metrics.classification_report(labels, preds_rounded, output_dict=True)

  #realiza a separação das imagens que deram falso positivos e falsos negativos: Erros da rede
  if os.path.exists("Falsos_positivos/sp_"+str(split+1)) == False: #cria a pasta do split correspondente
    os.mkdir("Falsos_positivos/sp_"+str(split+1))
  
  if os.path.exists("Falsos_positivos/sp_"+str(split+1)+"/normais") == False: #cria a pasta para os erros das normais
    os.mkdir("Falsos_positivos/sp_"+str(split+1)+"/normais")
   
  if os.path.exists("Falsos_positivos/sp_"+str(split+1)+"/melanomas") == False: #cria a pasta para os erros das melanomas
    os.mkdir("Falsos_positivos/sp_"+str(split+1)+"/melanomas")
  ...
  for i in range(0, test_samples): #realiza a cópia das imagens erradas para suas respectivas pastas
    if(preds_rounded[i] != labels[i]):
      ...
      copy(test_data_dir + '/' +test_gen.filenames[i], "Falsos_positivos/sp_"+str(split+1)+dest) #realização da cópia das imagens

  with open('Results/' + json_name, 'w') as outfile:  
    json.dump(metricas, outfile, indent=4, separators=(',', ': '))
